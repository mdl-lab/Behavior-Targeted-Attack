...
from policy_gradients.torch_utils import Identity
...
def main(params):
    ...
    if params["mode"] == "adv_ilfd" or params["mode"] == "adv_ilfo" or params["mode"] == "adv_ppo_ia" or params["mode"] == "adv_ppo" or params["mode"] == "adv_trpo" or params["mode"] == "adv_sa_ppo":
    ...
    def add_adversary_to_table(p, table_dict):
        if (
            params["mode"] == "adv_ilfd"
            or params["mode"] == "adv_ilfo"
            or params["mode"] == "adv_ppo_ia"
            or params["mode"] == "adv_ppo"
            or params["mode"] == "adv_trpo"
            or params["mode"] == "adv_sa_ppo"
        ):
    ...
    try:
        for i in range(params["train_steps"]):
            ...
            if params["save_iters"] > 0 and i % params["save_iters"] == 0 and i != 0:
                ...
                if params["save_normalization_factor"]:
                    experiment_id = os.path.basename(store.path)
                    os.makedirs(f"normalized_envs/history/{experiment_id}", exist_ok=True)
                    output_dir = os.path.join(f"normalized_envs/history/{experiment_id}")
                    with open(os.path.join(output_dir, params["game"] + ".pkl"), "wb") as f:
                        pickle.dump(p.envs, f)
    ...
    with open(os.path.join(store.path, "params.json"), "w") as f:
...
def add_common_parser_opts(parser):
    ...
    parser.add_argument("--mode", type=str, choices=["ppo", "trpo", "robust_ppo", "adv_ppo", "adv_trpo", "adv_sa_ppo", "adv_ilfd", "adv_ilfo", "adv_ppo_ia", "discounted_robust_ppo"], help="pg alg")
    parser.add_argument("--out-dir", type=str, help="out dir for store + logging")
    parser.add_argument("--advanced-logging", type=str2bool, const=True, nargs="?")
    parser.add_argument("--kl-approximation-iters", type=int, help="how often to do kl approx exps")
    parser.add_argument("--log-every", type=int)
    parser.add_argument("--policy-net-type", type=str, choices=models.POLICY_NETS.keys())
    parser.add_argument("--value-net-type", type=str, choices=models.VALUE_NETS.keys())
    parser.add_argument("--train-steps", type=int, help="num agent training steps")
    parser.add_argument("--cpu", type=str2bool, const=True, nargs="?")

    # Which value loss to use
    parser.add_argument("--value-calc", type=str, help="which value calculation to use")
    parser.add_argument("--initialization", type=str)

    # General Policy Gradient parameters
    parser.add_argument("--num-actors", type=int, help="num actors (serial)", choices=[1])
    parser.add_argument("--t", type=int, help="num timesteps to run each actor for")
    ...
    parser.add_argument("--num-minibatches", type=int, help="num minibatches in ppo per epoch")
    parser.add_argument("--ppo-epochs", type=int)
    parser.add_argument("--ppo-lr", type=float, help="if nonzero, use gradient descent w this lr")
    parser.add_argument("--ppo-lr-adam", type=float, help="if nonzero, use adam with this lr")
    parser.add_argument("--anneal-lr", type=str2bool, help="if we should anneal lr linearly from start to finish")
    ...
    parser.add_argument("--entropy-coeff", type=float, help="entropy weight hyperparam")
    parser.add_argument("--value-clipping", type=str2bool, help="should clip values (w/ ppo eps)")
    parser.add_argument("--value-multiplier", type=float, help="coeff for value loss in combined step ppo loss")
    parser.add_argument("--share-weights", type=str2bool, help="share weights in valnet and polnet")
    parser.add_argument("--clip-grad-norm", type=float, help="gradient norm clipping (-1 for no clipping)")
    parser.add_argument("--policy-activation", type=str, help="activation function for countinous policy network")
    ...
    parser.add_argument("--fisher-frac-samples", type=float, help="frac samples to use in fisher vp estimate")
    parser.add_argument("--cg-steps", type=int, help="num cg steps in fisher vp estimate")
    ...
    parser.add_argument("--adv-ppo-lr-adam", type=float, help="if nonzero, use adam for adversary policy with this lr")
    parser.add_argument("--adv-entropy-coeff", type=float, help="entropy weight hyperparam for adversary policy")
    ...
    parser.add_argument(
        "--attack-method",
        type=str,
        choices=["none", "critic", "random", "action", "sarsa", "sarsa+action", "advpolicy", "action+imit", "targetPGD"],
    ...
    parser.add_argument("--attack-targetpolicy-network", type=str, help="target policy network to load for target PGD attack.")
    parser.add_argument("--collect-perturbed-states", type=str2bool, help="collect perturbed states during training")

    # Normalization parameters
    parser.add_argument("--norm-rewards", type=str, help="type of rewards normalization", choices=["rewards", "returns", "none"])
    ...
    # Save normalized env
    parser.add_argument("--save-normalization-factor", type=bool, help="Record normalization and save normalized env to normalization_envs directory")

    # SAIA training parameters
    parser.add_argument("--action-noise-std", type=float, help="action noise for exploration")
    parser.add_argument("--adv-q-lr", type=float, help="learning rate for Q function")
    parser.add_argument("--adv-q-net-type", type=str, help="type of Q function")
    parser.add_argument("--adv-disc-lr", type=float, help="learning rate for discriminator")
    parser.add_argument("--batch-size", type=int, help="batch size for training")
    parser.add_argument("--buffer-size", type=int, help="buffer size for replay buffer")
    parser.add_argument("--demo-episode-size", type=int, help="number of episodes in expert buffer")
    parser.add_argument("--disc-gradient-steps", type=int, help="number of steps to train discriminator")
    parser.add_argument("--expert-buffer-path", type=str, help="path to expert buffer")
    parser.add_argument("--gp-lambda", type=float, help="lambda for gradient penalty")
    parser.add_argument("--gradient-steps", type=int, help="number of steps to train policy")
    parser.add_argument("--learning-starts", type=int, help="number of steps to start training")
    parser.add_argument("--tau", type=float, help="target network update rate")
    parser.add_argument("--noise-clip", type=float, help="clip range of noise for exploration")
    parser.add_argument("--policy-delay", type=int, help="delay for policy update")
...
def override_json_params(params, json_params, excluding_params):
    ...
    for key in params:
        if key not in json_params and key not in excluding_params:
            missing_keys.append(key)
    if missing_keys:
        warnings.warn("Following keys not in JSON: " + str(missing_keys))
...
if __name__ == "__main__":
    ...
    parser.add_argument("--config-path", type=str, required=True, help="json for this config")
    ...
    with open(args.config_path) as config_file:
        json_params = json.load(config_file)
