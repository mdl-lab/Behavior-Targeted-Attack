...
from .buffer import AdvReplayBuffer, ExpertBuffer
from .convex_relaxation import get_kl_bound as get_state_kl_bound
from .custom_env import Env
from .custom_logging import *
from .models import *
from .steps import pack_history, step_with_mode, value_step
...
class Trainer:
    ...
    def __init__(self, policy_net_class, value_net_class, params, store, advanced_logging=True, log_every=5):
        ...
        if not self.params.SAVE_NORMALIZATION_FACTOR:
            path = os.path.join("normalized_envs", self.params.game + ".pkl")
            print(f"Load normalization factors from {path}")
            with open(path, "rb") as f:
                self.envs = pickle.load(f)
            for e in self.envs:
                e.normalizer_read_only = True
        else:
            print("Record and save normalization factors.")
        ...
        # Instantiation
        if self.MODE == "discounted_robust_ppo":
            # States has time dimension in stepwise mode.
            for e in self.envs:
                e.state_filter = StateWithTime(e.state_filter, horizon=1)
            self.policy_model = policy_net_class(self.NUM_FEATURES, self.NUM_ACTIONS, self.INITIALIZATION, time_in_state=time_in_state, activation=self.policy_activation, stepwise=True)
        else:
            self.policy_model = policy_net_class(self.NUM_FEATURES, self.NUM_ACTIONS, self.INITIALIZATION, time_in_state=time_in_state, activation=self.policy_activation)

        # Instantiate convex relaxation model when mode is 'robust_ppo'
        if self.MODE == "discounted_robust_ppo" or self.MODE == "robust_ppo" or self.MODE == "adv_sa_ppo":
            self.create_relaxed_model(time_in_state)

        # Minimax training
        if self.MODE == "adv_ilfd" or self.MODE == "adv_ilfo" or self.MODE == "adv_ppo_ia" or self.MODE == "adv_ppo" or self.MODE == "adv_trpo" or self.MODE == "adv_sa_ppo":
            ...
            self.adversary_policy_model = policy_net_class(self.NUM_FEATURES, self.NUM_FEATURES, self.INITIALIZATION, time_in_state=time_in_state, activation=self.policy_activation)
            ...
            # Adversary function.
            if self.MODE == "adv_ilfd" or self.MODE == "adv_ilfo":
                # Set action noise and noise clip
                self.adversary_policy_model.log_stdev.data.fill_(self.ACTION_NOISE_STD)
                self.adversary_policy_model.clip_noise = self.NOISE_CLIP
                # Adversary Q function
                q_net_class = value_net_with_name(self.ADV_Q_NET_TYPE)  # value net is used for pre-trained models, so q net is defined here.
                self.adversary_Q_model = q_net_class(self.NUM_FEATURES, self.NUM_FEATURES, self.INITIALIZATION)  # adversary's action space is equal to state dim
                if self.ADV_Q_LR == "same":
                    self.ADV_Q_LR = self.ADV_PPO_LR_ADAM
                self.params.ADV_Q_ADAM = optim.Adam(self.adversary_Q_model.parameters(), lr=self.ADV_Q_LR, eps=1e-5)
                # Adversary discriminator
                if self.MODE == "adv_ilfd":
                    # In ILfD, the discriminator is used to distinguish between states and actions.
                    self.adversary_disc_model = CtsDiscNet(self.NUM_FEATURES, self.NUM_ACTIONS, self.INITIALIZATION)
                elif self.MODE == "adv_ilfo":
                    # In ILfO, the discriminator is used to distinguish between states and next states.
                    self.adversary_disc_model = CtsDiscNet(self.NUM_FEATURES, self.NUM_FEATURES, self.INITIALIZATION)
                else:
                    raise NotImplementedError
                self.params.ADV_DISC_ADAM = optim.Adam(self.adversary_disc_model.parameters(), lr=self.ADV_DISC_LR, eps=1e-5)
                # Adversary target policy function.
                self.adversary_target_policy_model = policy_net_class(self.NUM_FEATURES, self.NUM_FEATURES, self.INITIALIZATION, time_in_state=time_in_state, activation=self.policy_activation)
                self.adversary_target_policy_model.load_state_dict(self.adversary_policy_model.state_dict())
                # Adversary target Q function.
                self.adversary_target_Q_model = q_net_class(self.NUM_FEATURES, self.NUM_FEATURES, self.INITIALIZATION)
                self.adversary_target_Q_model.load_state_dict(self.adversary_Q_model.state_dict())
                # Adversary replay buffers.
                self.adv_replay_buffer = AdvReplayBuffer(self.BUFFER_SIZE, self.NUM_FEATURES, self.NUM_ACTIONS)  # Adversary's action space is equal to state dim
                with open(self.EXPERT_BUFFER_PATH, "rb") as f:
                    expert_demo = dill.load(f)
                self.adv_expert_buffer = expert_demo.create_buffer(self.DEMO_EPISODE_SIZE)

                # Dummy val model for save adversary
            ...
            # Learning rate annealing for adversary.
            if self.ANNEAL_LR:
                adv_lam = lambda f: 1 - f / self.TRAIN_STEPS
                adv_ps = optim.lr_scheduler.LambdaLR(self.ADV_POLICY_ADAM, lr_lambda=adv_lam)
                self.params.ADV_POLICY_SCHEDULER = adv_ps
                if self.MODE == "adv_ilfd" or self.MODE == "adv_ilfo":
                    adv_q = optim.lr_scheduler.LambdaLR(self.ADV_Q_ADAM, lr_lambda=adv_lam)
                    adv_disc = optim.lr_scheduler.LambdaLR(self.ADV_DISC_ADAM, lr_lambda=adv_lam)
                    self.params.ADV_Q_SCHEDULER = adv_q
                    self.params.ADV_DISC_SCHEDULER = adv_disc
                else:
        ...
        if self.PPO_LR_ADAM != -1:
            ...
            self.params.POLICY_ADAM = optim.Adam(self.policy_model.parameters(), **kwargs)
        else:
        ...
        if self.MODE == "discounted_robust_ppo":
            self.val_model = value_net_class(self.NUM_FEATURES, self.INITIALIZATION, stepwise=True)
        else:
        ...
        if self.ANNEAL_LR:
            ...
            ps = optim.lr_scheduler.LambdaLR(self.POLICY_ADAM, lr_lambda=lam)
    ...
    def setup_imit(self, train=True, lr=1e-3):
        ...
        self.imit_network = self.policy_net_class(self.NUM_FEATURES, self.NUM_ACTIONS, self.INITIALIZATION, time_in_state=self.VALUE_CALC == "time", activation=self.policy_activation)
        if train:
            if self.PPO_LR_ADAM != -1:
                kwargs = {
                    "lr": lr,
    ...
    def setup_stores(self, store):
        ...
        if self.MODE == "adv_ppo_ia" or self.MODE == "adv_ppo" or self.MODE == "adv_trpo" or self.MODE == "adv_sa_ppo":
            adv_optimization_table = {
                "mean_reward": float,
        ...
        if self.advanced_logging:
            paper_constraint_cols = {"avg_kl": float, "max_kl": float, "max_ratio": float, "opt_step": int}

            value_cols = {"heldout_gae_loss": float, "heldout_returns_loss": float, "train_gae_loss": float, "train_returns_loss": float}
            ...
            self.store.add_table("paper_constraints_train", paper_constraint_cols)
            self.store.add_table("paper_constraints_heldout", paper_constraint_cols)
            self.store.add_table("value_data", value_cols)
            self.store.add_table("weight_updates", weight_cols)

        if self.params.MODE == "discounted_robust_ppo" or self.params.MODE == "robust_ppo" or self.params.MODE == "adv_sa_ppo":
            robust_cols = {
                "eps": float,
                "beta": float,
                "kl": float,
    ...
    def advantage_and_return(self, rewards, values, not_dones):
        ...
        for agent, start, end in indices:
            advantages[agent, start:end] = discount_path(deltas[agent, start:end], self.LAMBDA * self.GAMMA)
            returns[agent, start:end] = discount_path(rewards[agent, start:end], self.GAMMA)
    ...
    def run_trajectories(self, num_saps, return_rewards=False, should_tqdm=False, collect_adversary_trajectory=False):
        """
        ...
        if collect_adversary_trajectory:
            ...
            assert self.MODE == "adv_ilfd" or self.MODE == "adv_ilfo" or self.MODE == "adv_ppo_ia" or self.MODE == "adv_ppo" or self.MODE == "adv_trpo" or self.MODE == "adv_sa_ppo"
        ...
        victim_actions_shape = shape + (self.NUM_ACTIONS,)
        victim_actions = ch.zeros(victim_actions_shape)

        states_shape = (self.NUM_ACTORS, traj_length + 1) + initial_states.shape[2:]
        states = ch.zeros(states_shape)
        next_states_list = ch.zeros(states_shape)

        iterator = range(traj_length) if not should_tqdm else tqdm.trange(traj_length)

        assert self.NUM_ACTORS == 1

        is_advpolicy_training = self.MODE == "adv_ilfd" or self.MODE == "adv_ilfo" or self.MODE == "adv_ppo_ia" or self.MODE == "adv_ppo" or self.MODE == "adv_trpo" or self.MODE == "adv_sa_ppo"

        collect_perturbed_state = (is_advpolicy_training and not collect_adversary_trajectory) or ((not is_advpolicy_training) and self.COLLECT_PERTURBED_STATES)
            ...
            if collect_adversary_trajectory:
                if self.MODE == "adv_ilfd":
                    pairs = [
                        ...
                        (next_states_list, next_states),
                        (victim_actions, next_actions),  # The victim actions. This is used only for SAIA ILfD.
                    ]
                else:
                    # negate the reward for minimax training. Collect states before perturbation.
                    if not self.MODE == "adv_ppo_ia":  # in SA-RL for imitation attack, rewards for adversary is just the true reward.
        ...
        # print(infos)
        if infos.size > 0:
            _, ep_rewards, _ = infos
            avg_episode_length, avg_episode_reward, adv_episode_success = np.mean(infos, axis=1)
        else:
            ep_rewards = [-1]
            avg_episode_length = -1
            avg_episode_reward = -1

        next_states_list = states[:, 1:, :]
    ...
    def apply_attack(self, last_states):
        ...
        if self.params.ATTACK_METHOD == "critic":
            ...
            if not hasattr(self, "attack_policy_network"):
                self.attack_policy_network = self.policy_net_class(
                    self.NUM_FEATURES,
                    self.NUM_FEATURES,
                    self.INITIALIZATION,
                    time_in_state=self.VALUE_CALC == "time",
        ...
        elif self.params.ATTACK_METHOD == "targetPGD":
            # Ensure that target_policy is defined
            assert self.params.ATTACK_TARGETPOLICY_NETWORK is not None
            if not hasattr(self, "target_policy_network"):
                self.target_policy_network = self.policy_net_class(
                    self.NUM_FEATURES,
                    self.NUM_ACTIONS,
                    self.INITIALIZATION,
                    time_in_state=self.VALUE_CALC == "time",
                ...
                print("Loading target policy network", self.params.ATTACK_TARGETPOLICY_NETWORK)
                target_policy_ckpt = torch.load(self.params.ATTACK_TARGETPOLICY_NETWORK)
                self.target_policy_network.load_state_dict(target_policy_ckpt["policy_model"])
            steps = self.params.ATTACK_STEPS
            steps = 30
            if steps > 0:
                if self.params.ATTACK_STEP_EPS == "auto":
                ...
                noise = torch.empty_like(last_states).uniform_(-step_eps, step_eps)
                states = last_states + noise
                # Target policy action at this state
                target_action, target_stdev = self.target_policy_network(states)
                target_stdev /= target_stdev.mean()
                target_action = target_action.detach()

                with torch.enable_grad():
                    for i in range(steps):
                        states = states.clone().detach().requires_grad_()
                        # Get the current action from the victim policy network
                        action, _ = self.policy_model(states)
                        # Compute the loss as the distance between the current action and the target action
                        action_change = (action - target_action) / target_stdev
                        loss = (action_change * action_change).mean(dim=1)

                        # loss = torch.norm(action - target_action, p=2, dim=1)
                        loss.backward()

                        # Apply the perturbation step with the gradient sign method
                        update = states.grad.sign() * eps
                        # Clamp the updated states within the allowed epsilon range
                        # states.data = torch.min(torch.max(states.data - update, clamp_min), clamp_max)
    ...
    def collect_saps(self, num_saps, should_log=True, return_rewards=False, should_tqdm=False, test=False, collect_adversary_trajectory=False):
        table_name_suffix = "_adv" if collect_adversary_trajectory else ""
        with torch.no_grad():
            # Run trajectories, get values, estimate advantage
            output = self.run_trajectories(num_saps, return_rewards=return_rewards, should_tqdm=should_tqdm, collect_adversary_trajectory=collect_adversary_trajectory)
            ...
            # No need to compute advantage function for TD3 algorithm.
            if not test and self.MODE != "adv_ilfd" and self.MODE != "adv_ilfo":
                ...
                advantages, returns = self.advantage_and_return(trajs.rewards, values, trajs.not_dones)

                trajs.advantages = advantages
                trajs.returns = returns
                trajs.values = values

                assert shape_equal_cmp(trajs.advantages, trajs.returns, trajs.values)
            ...
            if should_log:
                ...
                if not test:
                    self.store.log_table_and_tb("optimization" + table_name_suffix, {"mean_reward": avg_ep_reward})
    ...
    def take_steps(self, saps, logging=True, value_only=False, adversary_step=False, increment_scheduler=True):
        if adversary_step:
            # collect adversary trajectory is only valid in minimax training mode.
            assert self.MODE == "adv_ppo_ia" or self.MODE == "adv_ppo" or self.MODE == "adv_trpo" or self.MODE == "adv_sa_ppo"

        # Begin advanged logging code
        assert saps.unrolled
        should_adv_log = self.advanced_logging and self.n_steps % self.log_every == 0 and logging

        self.params.SHOULD_LOG_KL = self.advanced_logging and self.KL_APPROXIMATION_ITERS != -1 and self.n_steps % self.KL_APPROXIMATION_ITERS == 0
        ...
        if logging:
            self.store.log_table_and_tb("optimization" + table_name_suffix, {"final_value_loss": val_loss})

        if (self.MODE == "discounted_robust_ppo" or self.MODE == "robust_ppo" or self.MODE == "adv_sa_ppo") and not adversary_step and logging:
        ...
        if (self.MODE == "discounted_robust_ppo" or self.MODE == "robust_ppo" or self.MODE == "adv_sa_ppo") and isinstance(self.policy_model, CtsPolicy) and not adversary_step:
        ...
        if should_adv_log and not adversary_step:
            ...
            paper_constraints_logging(self, saps, old_pds, table="paper_constraints_train")
            paper_constraints_logging(self, val_saps, val_old_pds, table="paper_constraints_heldout")
        ...
        if (self.params.MODE == "discounted_robust_ppo" or self.params.MODE == "robust_ppo" or self.params.MODE == "adv_sa_ppo") and not adversary_step:
    ...
    def take_steps_saia(self, saps, logging=True, value_only=False, adversary_step=False, increment_scheduler=True):
        if adversary_step:
            # collect adversary trajectory is only valid in minimax training mode.
            assert self.MODE == "adv_ilfd" or self.MODE == "adv_ilfo"
        ...
        if adversary_step:
            ...
            if self.ANNEAL_LR:
                policy_scheduler = self.ADV_POLICY_SCHEDULER
                q_scheduler = self.ADV_Q_SCHEDULER
                disc_scheduler = self.ADV_DISC_SCHEDULER
            policy_params = Parameters(self.params.copy())
            # In policy_step(), some hard coded attributes will be accessed. We override them.
            policy_params.PPO_LR = self.ADV_PPO_LR_ADAM
            policy_params.PPO_LR_ADAM = self.ADV_PPO_LR_ADAM
            ...
            policy_params.ENTROPY_COEFF = policy_params.ADV_ENTROPY_COEFF
            table_name_suffix = "_adv"
            q_model = self.adversary_Q_model
            policy_target_model = self.adversary_target_policy_model
            q_target_model = self.adversary_target_Q_model
            policy_params.Q_ADAM = self.ADV_Q_ADAM
            disc_model = self.adversary_disc_model
            policy_params.DISC_ADAM = self.ADV_DISC_ADAM
            # add experiences to replay buffer
            self.adv_replay_buffer.add(saps.states, saps.next_states, saps.actions, saps.victim_actions, saps.rewards, saps.not_dones)
            replay_buffer = self.adv_replay_buffer
            expert_buffer = self.adv_expert_buffer
        else:
            policy_model = self.policy_model
            if self.ANNEAL_LR:
                policy_scheduler = self.POLICY_SCHEDULER
        ...
        # Policy optimization step
        if adversary_step:
            if replay_buffer.get_size() > self.LEARNING_STARTS:
                policy_loss, q_loss, disc_loss = self.adversary_policy_step(*args)
            else:
                print(f"Skipping policy training because number of experiences is less than learning_starts: {replay_buffer.get_size()} / {self.LEARNING_STARTS}")
                policy_loss, q_loss, disc_loss = 0, 0, 0
        else:
            policy_loss, surr_loss, entropy_bonus = self.policy_step(*args)
        ...
        if self.ANNEAL_LR:
            print(f"policy lr: {policy_scheduler.get_last_lr()}, q lr: {q_scheduler.get_last_lr()}, disc lr: {disc_scheduler.get_last_lr()}")

        return policy_loss, q_loss, disc_loss
    ...
    def train_step_impl(self, adversary_step=False, increment_scheduler=True):
        ...
        if adversary_step and (self.MODE == "adv_ilfd" or self.MODE == "adv_ilfo"):
            policy_loss, q_loss, disc_loss = self.take_steps_saia(saps, adversary_step=adversary_step, increment_scheduler=increment_scheduler)
            # Logging code
            print(f"Policy Loss: {policy_loss:.5g}, | Q Loss: {q_loss:.5g}, | Disc Loss: {disc_loss:.5g}")
            print("Time elapsed (s):", time.time() - start_time)
            # Check model's stdev is 0 for deterministic.
            mean_std = ch.exp(policy_model.log_stdev).mean()
            print("Agent stdevs: %s" % mean_std.detach().cpu().numpy())
            self.store.log_table_and_tb(
                "optimization" + table_name_suffix,
                {
                    "mean_std": mean_std,
    ...
    def run_test(self, max_len=2048, compute_bounds=False, use_full_backward=False, original_stdev=None):
        ...
        with torch.no_grad():
            ...
            ep_length, ep_reward, ep_success, actions, action_means, states = output
            msg = "Episode reward: %f | episode length: %f | episode success: %f"
            print(msg % (ep_reward, ep_length, ep_success))
            if compute_bounds:
                ...
                kl_upper_bound = get_state_kl_bound(self.relaxed_policy_model, states, action_means, eps=eps, beta=0.0, stdev=kl_stdev, use_full_backward=use_full_backward).mean()
                kl_upper_bound = kl_upper_bound.item()
            else:
                kl_upper_bound = float("nan")
            # Unroll the trajectories (actors, T, ...) -> (actors*T, ...)
        return ep_length, ep_reward, ep_success, actions.cpu().numpy(), action_means.cpu().numpy(), states.cpu().numpy(), kl_upper_bound
    ...
    def run_test_trajectories(self, max_len, should_tqdm=False):
        ...
        if len(completed_episode_info) > 0:
            ep_length, ep_reward, ep_success = completed_episode_info[0]
        else:
            ep_length = np.nan
            ep_reward = np.nan
        ...
        to_ret = (ep_length, ep_reward, ep_success, actions, action_means, states)

        return to_ret

    def run_collect_demo(self, max_len=2048):
        print("-" * 80)

        output = self.run_collect_demo_trajectories(max_len)
        ep_length, ep_reward, actions, action_means, states, next_states, rewards, not_dones = output

        return ep_length, ep_reward, actions, states, next_states, rewards, not_dones

    def run_collect_demo_trajectories(self, max_len, should_tqdm=False):
        # Arrays to be updated with historic info
        envs = self.envs
        initial_states = self.reset_envs(envs)
        if hasattr(self, "imit_network"):
        ...
        if len(completed_episode_info) > 0:
            ep_length, ep_reward, _ = completed_episode_info[0]
        else:
            ep_length = np.nan
            ep_reward = np.nan
        ...
        action_means = action_means[0][: t + 1]
        states = states[0][: t + 1]
        next_states_list = next_states_list[0][: t + 1]
        rewards = rewards[0][: t + 1]
        not_dones = not_dones[0][: t + 1]

        to_ret = (ep_length, ep_reward, actions, action_means, states, next_states_list, rewards, not_dones)
    ...
    def agent_from_params(params, store=None):
        ...
        p = Trainer(agent_policy, agent_value, params, store, log_every=log_every, advanced_logging=advanced_logging)
