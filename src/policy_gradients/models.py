...
class ValueDenseNet(nn.Module):
    ...
    def __init__(self, state_dim, init=None, hidden_sizes=(64, 64), activation=None, stepwise=False):
        """
    ...
    def forward(self, x):
        ...
        """
        # In stepwise mode, states has time dimension in training mode.
        if self.stepwise and x.shape[-1] > self.affine_layers[0].in_features:
            x = x[..., :-1]
...
class ValueLSTMNet(nn.Module):
    ...
    def __init__(self, state_dim, init=None, hidden_sizes=(64, 64), activation=None):
        ...
        self.hidden = [ch.zeros(1, 1, self.hidden_sizes[1]), ch.zeros(1, 1, self.hidden_sizes[1])]
        self.paused = False
...
class CtsPolicy(nn.Module):
    ...
    def __init__(self, state_dim, action_dim, init, hidden_sizes=HIDDEN_SIZES, time_in_state=False, share_weights=False, activation=None, use_merged_bias=False, stepwise=False):
        super().__init__()
        if isinstance(activation, str):
            self.activation = activation_with_name(activation)()
        else:
    ...
    def forward(self, x):
        # In stepwise robust ppo, states has time dimension in training mode, so we need to remove the last element.
        if self.stepwise and x.shape[-1] > self.affine_layers[0].in_features + self.time_in_state:
            x = x[..., :-1]
        # If the time is in the state, discard it
        if self.time_in_state:
            x = x[:, :-1]
    ...
    def get_loglikelihood(self, p, actions):
        try:
            mean, std = p
            nll = 0.5 * ((actions - mean) / std).pow(2).sum(-1) + 0.5 * np.log(2.0 * np.pi) * actions.shape[-1] + self.log_stdev.sum(-1)
            return -nll
        except Exception as e:
            raise ValueError("Numerical error")
    ...
    def entropies(self, p):
        ...
        d = std.shape[0]
        entropies = ch.log(detp) + 0.5 * (d * (1.0 + math.log(2 * math.pi)))
        return entropies
...
class CtsLSTMPolicy(CtsPolicy):
    ...
    def __init__(self, state_dim, action_dim, init, hidden_sizes=HIDDEN_SIZES, time_in_state=False, share_weights=False, activation=None, use_merged_bias=False):
...
class CtsPolicyLarger(CtsPolicy):
    ...
    def __init__(self, state_dim, action_dim, init, time_in_state=False, share_weights=False, activation=None, use_merged_bias=False):
        super().__init__(state_dim, action_dim, init, hidden_sizes=[400, 300], time_in_state=False, share_weights=False, activation="relu", use_merged_bias=False)
...
class CtsPolicySAC(CtsPolicy):
    ...
    def __init__(self, state_dim, action_dim, init, time_in_state=False, share_weights=False, activation=None, use_merged_bias=False):
        super().__init__(state_dim, action_dim, init, hidden_sizes=[256, 256], time_in_state=False, share_weights=False, activation="relu", use_merged_bias=False)
...
class CtsPolicyTD3(CtsPolicy):

    def __init__(self, state_dim, action_dim, init, time_in_state=False, share_weights=False, activation=None, use_merged_bias=False):
        super().__init__(state_dim, action_dim, init, hidden_sizes=HIDDEN_SIZES, time_in_state=False, share_weights=False, activation="relu", use_merged_bias=False)

        # Since TD3 algorithm uses deterministic policy, set log_stdev to zero and make it non-trainable
        self.log_stdev.data.fill_(-100)
        self.log_stdev.requires_grad = False

        self.noise_clip = 0

    def forward(self, x):
        mean, std = super().forward(x)
        std = std.detach()  # Ensure std is not trainable
        return ch.tanh(mean), std  # TD3 is deterministic, so we don't need std
    ...
    def sample(self, p):
        ...
        """
        means, std = p
        noise = ch.randn_like(means) * std
        return (means + noise).detach()


class CtsDoubleQNet(nn.Module):

    def __init__(self, state_dim, action_dim, init=None, hidden_sizes=HIDDEN_SIZES, activation=None):
        super().__init__()
        if isinstance(activation, str):
            self.activation = activation_with_name(activation)()
        else:
        ...
        self.affine_layers1 = nn.ModuleList()
        self.affine_layers2 = nn.ModuleList()
        prev_size = state_dim + action_dim

        for i in hidden_sizes:
            lin1 = nn.Linear(prev_size, i)
            lin2 = nn.Linear(prev_size, i)
            if init is not None:
                initialize_weights(lin1, init)
                initialize_weights(lin2, init)
            self.affine_layers1.append(lin1)
            self.affine_layers2.append(lin2)
            prev_size = i

        self.final1 = nn.Linear(prev_size, 1)
        self.final2 = nn.Linear(prev_size, 1)
        if init is not None:
            initialize_weights(self.final1, init, scale=1.0)
            initialize_weights(self.final2, init, scale=1.0)

    def initialize(self, init="orthogonal"):
        for layer1, layer2 in zip(self.affine_layers1, self.affine_layers2):
            initialize_weights(layer1, init)
            initialize_weights(layer2, init)
        initialize_weights(self.final1, init, scale=1.0)
        initialize_weights(self.final2, init, scale=1.0)

    def forward(self, states, actions):
        x1 = ch.cat([states, actions], dim=1)
        x2 = ch.cat([states, actions], dim=1)
        for affine1, affine2 in zip(self.affine_layers1, self.affine_layers2):
            x1 = self.activation(affine1(x1))
            x2 = self.activation(affine2(x2))
        q1 = self.final1(x1)
        q2 = self.final2(x2)
        return q1, q2
    ...
    def q1_forward(self, states, actions):
        x1 = ch.cat([states, actions], dim=1)
        for affine1 in self.affine_layers1:
            x1 = self.activation(affine1(x1))
        q1 = self.final1(x1)
        return q1


class CtsDiscNet(nn.Module):

    def __init__(self, state_dim, feature_dim, init=None, hidden_sizes=HIDDEN_SIZES, activation=None):
        super().__init__()
        if isinstance(activation, str):
            self.activation = activation_with_name(activation)()
        else:
        ...
        self.affine_layers = nn.ModuleList()
        prev_size = state_dim + feature_dim
        for i in hidden_sizes:
            lin = nn.Linear(prev_size, i)
            if init is not None:
                initialize_weights(lin, init)
        ...
        self.final = nn.Linear(prev_size, 1)
        if init is not None:
            initialize_weights(self.final, init, scale=1.0)

        # Sigmoid layer to constrain output to [0, 1]
        self.output_activation = nn.Sigmoid()

    def initialize(self, init="orthogonal"):
        for layer in self.affine_layers:
            initialize_weights(layer, init)
        initialize_weights(self.final, init, scale=1.0)
...
POLICY_NETS = {
    ...
    "CtsPolicyTD3": CtsPolicyTD3,
}

VALUE_NETS = {
    "ValueNet": ValueDenseNet,
    "DoubleQNet": CtsDoubleQNet,
}
