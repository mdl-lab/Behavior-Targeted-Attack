...
def cpu_tensorize(t):
    ...
    - Tensor version of t
    """
    if isinstance(t, list) and isinstance(t[0], np.ndarray):
        t = np.array(t)  # Convert list to numpy array
    ...
    for arg in args:
        if list(arg.shape) != list(a):
            if len(arg.shape) != len(a):
                raise ValueError("Expected shape: %s, Got shape %s" % (str(a), str(arg.shape)))
...
def backtracking_line_search(f, x, expected_improve_rate, num_tries=10, accept_ratio=0.1):
    """
...
class Trajectories:
    def __init__(
        ...
        self.states = states
        self.next_states = next_states  # Added
        ...
        self.victim_actions = victim_actions
    ...
    def tensor_op(self, lam, should_wrap=True):
        ...
        tt = op(self.states, self.next_states, self.rewards, self.returns, self.not_dones)  # Added next_states
        tt2 = op(self.actions, self.action_log_probs, self.advantages, self.action_means, self.victim_actions)
        (values,) = op(self.values)

        ts = Trajectories(
            states=tt[0],
            next_states=tt[1],  # Added
            rewards=tt[2],
            returns=tt[3],
            not_dones=tt[4],
            actions=tt2[0],
            action_log_probs=tt2[1],
            advantages=tt2[2],
            action_means=tt2[3],
            action_std=self.action_std,
            values=values,
            unrolled=True,
            victim_actions=tt2[4],
        )
