...
from .custom_metaworld import custom_metaworld
from .custom_mujoco import custom_mujoco
from .torch_utils import Identity, RewardFilter, RunningStat, StateWithTime, ZFilter
...
class Env:
    ...
    def __init__(
        ...
        if game in _env_dict.ALL_V2_ENVIRONMENTS:
            self.env = custom_metaworld(game)
        if "Ant" in game or "HalfCheetah" in game or "Hopper" in game or "Humanoid" in game or "Walker" in game:
            self.env = custom_mujoco(game)
        else:
        ...
        self.num_actions = self.env.action_space.n if self.is_discrete else 0 if len(action_shape) == 0 else action_shape[0]
    ...
    def step(self, action):
        ...
        if is_done:
            if "success" in info:
                info["done"] = (self.counter, self.total_true_reward, info["success"])
            else:
                info["done"] = (self.counter, self.total_true_reward, False)
...
if __name__ == "__main__":
    env = Env("window-close-v2", norm_states=True, norm_rewards="rewards", params=None, clip_obs=10.0, clip_rew=10.0)
    env.reset()

    state, reward, is_done, info = env.step(env.env.action_space.sample())
