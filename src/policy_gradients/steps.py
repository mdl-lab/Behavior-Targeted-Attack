...
def adv_normalize(adv, mask=None):
    ...
    assert std != 0.0 and not ch.isnan(std), "Need nonzero std"
...
def value_loss_returns(vs, returns, advantages, not_dones, params, old_vs, mask=None, store=None, re=False):
    """
...
def value_step(
    ...
    VALUE_FUNCS = {"gae": value_loss_gae, "time": value_loss_returns}
    ...
    r = range(params.VAL_EPOCHS) if not should_tqdm else tqdm(range(params.VAL_EPOCHS))
    ...
    for i in r:
        if params.HISTORY_LENGTH > 0 and params.USE_LSTM_VAL:
            ...
            for i, batch in enumerate(batches):
                ...
                batch_val_loss = vf(vs, batch_returns, batch_advs, batch_not_dones, params, batch_old_vs, mask=mask, store=store, reduction="sum")
            ...
            for selected in splits:
                ...
                vf = VALUE_FUNCS[params.VALUE_CALC]
                val_loss = vf(vs, sel_rets, sel_advs, sel_not_dones, params, sel_ovs, mask=mask, store=store)
        ...
        if should_tqdm:
            if test_saps is not None:
                ...
                test_loss = vf(vs, test_saps.returns, test_saps.advantages, test_saps.not_dones, params, old_test_vs, None)
            r.set_description(f"vf_train: {val_loss.mean().item():.2f}" f"vf_test: {test_loss.mean().item():.2f}")
...
def ilfd_step(replay_buffer, expert_buffer, policy_net, policy_net_target, q_net, q_net_target, disc_net, params, store):
    ...
    for _ in range(params.DISC_GRADIENT_STEPS):
        ...
        expert_batch = expert_buffer.sample(params.BATCH_SIZE)

        # Create inputs
        generator_inputs = ch.cat((batch.states, batch.victim_actions), dim=1)
        expert_inputs = ch.cat((expert_batch.states, expert_batch.victim_actions), dim=1)

        # Get random interpolation coefficients
        alpha = ch.rand((generator_inputs.size(0), 1), device=generator_inputs.device)
        interpolates = alpha * generator_inputs + (1.0 - alpha) * expert_inputs

        # generator_inputs.requires_grad_(True)
        # expert_inputs.requires_grad_(True)
        interpolates.requires_grad_(True)

        # Compute discriminator output for both inputs
        generator_output = disc_net(generator_inputs)
        expert_output = disc_net(expert_inputs)

        # Compute the loss
        accuracy_loss = generator_output.mean() - expert_output.mean()

        # Gradient penalty
        interpolate_output = disc_net(interpolates)
        interpolates_grads = ch.autograd.grad(
            outputs=interpolate_output,
            inputs=interpolates,
            grad_outputs=ch.ones_like(interpolate_output),
            create_graph=True,
            retain_graph=True,
            only_inputs=True,
        )[0]
        gradient_penalty = ((interpolates_grads.norm(2, dim=1) - 1) ** 2).mean()

        # Final loss
        discriminator_loss = accuracy_loss + params.GP_LAMBDA * gradient_penalty
        discriminator_losses.append(discriminator_loss.item())

        # Optimize the discriminator networks
        if params.DISC_ADAM is None:
            grad = ch.autograd.grad(discriminator_loss, disc_net.parameters())
            flat_grad = flatten(grad)
            if params.CLIP_GRAD_NORM != -1:
                norm_grad = ch.norm(flat_grad)
                flat_grad = flat_grad if norm_grad <= params.CLIP_GRAD_NORM else flat_grad / norm_grad * params.CLIP_GRAD_NORM

            assign(flatten(disc_net.parameters()) - params.PPO_LR * flat_grad, disc_net.parameters())
        else:
            params.DISC_ADAM.zero_grad()
            discriminator_loss.backward()
            if params.CLIP_GRAD_NORM != -1:
                ch.nn.utils.clip_grad_norm(disc_net.parameters(), params.CLIP_GRAD_NORM)
            params.DISC_ADAM.step()

    ### Update rewards to Discriminator outputs in replay buffer ###

    replay_buffer.update_rewards_with_discriminator(disc_net, mode="ilfd")

    ### Policy training ###

    policy_losses, q_losses = [], []
    for n_updates in range(params.GRADIENT_STEPS):
        # Sample replay buffer
        batch = replay_buffer.sample(params.BATCH_SIZE)
        ...
        with ch.no_grad():
            ...
            next_q_values = ch.cat([q1_target, q2_target], dim=1)
        ...
        if params.Q_ADAM is None:
            grad = ch.autograd.grad(q_loss, q_net.parameters())
            flat_grad = flatten(grad)
            if params.CLIP_GRAD_NORM != -1:
                norm_grad = ch.norm(flat_grad)
                flat_grad = flat_grad if norm_grad <= params.CLIP_GRAD_NORM else flat_grad / norm_grad * params.CLIP_GRAD_NORM

            assign(flatten(q_net.parameters()) - params.PPO_LR * flat_grad, q_net.parameters())
        else:
            params.Q_ADAM.zero_grad()
            q_loss.backward()
            if params.CLIP_GRAD_NORM != -1:
                ch.nn.utils.clip_grad_norm(q_net.parameters(), params.CLIP_GRAD_NORM)
            params.Q_ADAM.step()

        # Delayed policy updates
        if n_updates % params.POLICY_DELAY == 0:
            # Compute policy loss
            actions, _ = policy_net(batch.states)  # Get the action without noise via forward method
            policy_loss = -q_net.q1_forward(batch.states, actions).mean()
            assert isinstance(policy_loss, ch.Tensor)
            policy_losses.append(policy_loss.item())

            # Optimize the policy network
            if params.POLICY_ADAM is None:
                grad = ch.autograd.grad(policy_loss, policy_net.parameters())
                flat_grad = flatten(grad)
                if params.CLIP_GRAD_NORM != -1:
                    norm_grad = ch.norm(flat_grad)
                    flat_grad = flat_grad if norm_grad <= params.CLIP_GRAD_NORM else flat_grad / norm_grad * params.CLIP_GRAD_NORM

                assign(flatten(policy_net.parameters()) - params.PPO_LR * flat_grad, policy_net.parameters())
            else:
                params.POLICY_ADAM.zero_grad()
                policy_loss.backward()
                if params.CLIP_GRAD_NORM != -1:
                    ch.nn.utils.clip_grad_norm(policy_net.parameters(), params.CLIP_GRAD_NORM)
                params.POLICY_ADAM.step()

            # Update target networks
            with ch.no_grad():
                for target_param, param in zip(q_net_target.parameters(), q_net.parameters()):
                    target_param.data.mul_(1 - params.TAU)
                    target_param.data.add_(params.TAU * param.data)
                for target_param, param in zip(policy_net_target.parameters(), policy_net.parameters()):
                    target_param.data.mul_(1 - params.TAU)
                    target_param.data.add_(params.TAU * param.data)

    print(f"policy_loss={np.mean(policy_losses):8.5f}, Q_loss={np.mean(q_losses):8.5f}, disc_loss={np.mean(discriminator_losses):8.5f}")

    return np.mean(policy_losses), np.mean(q_losses), np.mean(discriminator_losses)


def ilfo_step(replay_buffer, expert_buffer, policy_net, policy_net_target, q_net, q_net_target, disc_net, params, store):
    ...
    for _ in range(params.DISC_GRADIENT_STEPS):
        ...
        # Create inputs
        generator_inputs = ch.cat((batch.states, batch.next_states), dim=1)
        expert_inputs = ch.cat((expert_batch.states, expert_batch.next_states), dim=1)
    ...
    replay_buffer.update_rewards_with_discriminator(disc_net, mode="ilfo")
...
def ppo_step(all_states, actions, old_log_ps, rewards, returns, not_dones, advs, net, params, store, opt_step):
    """
    ...
    for _ in range(params.PPO_EPOCHS):
        if params.HISTORY_LENGTH > 0:
            ...
            for i, batch in enumerate(batches):
                ...
                clp_rew = surrogate_reward(batch_advs, new=new_log_ps, old=batch_old_log_ps, clip_eps=params.CLIP_EPS, mask=mask, normalize=False)
            ...
            for selected in splits:
                ...
                clp_rew = surrogate_reward(batch_advs, new=new_log_ps, old=batch_old_log_ps, clip_eps=params.CLIP_EPS)
    ...
    for _ in range(params.PPO_EPOCHS):
        if params.HISTORY_LENGTH > 0:
            ...
            if params.ROBUST_PPO_METHOD == "sgld":
                kl_upper_bound = get_state_kl_bound_sgld(net, all_states, None, eps=current_eps, steps=params.ROBUST_PPO_PGD_STEPS, stdev=stdev, not_dones=not_dones).mean()
            else:
            ...
            for selected in splits:
                ...
                if params.ROBUST_PPO_METHOD == "convex-relax":
                    kl_upper_bound = get_state_kl_bound(relaxed_net, batch_states, batch_action_means, eps=current_eps, beta=beta_scheduler.get_eps(), stdev=stdev).mean()
                elif params.ROBUST_PPO_METHOD == "sgld":
                    kl_upper_bound = get_state_kl_bound_sgld(net, batch_states, batch_action_means, eps=current_eps, steps=params.ROBUST_PPO_PGD_STEPS, stdev=stdev).mean()
                else:
                    raise ValueError(f"Unsupported robust PPO method {params.ROBUST_PPO_METHOD}")
        ...
        print(
            f"eps={eps_scheduler.get_eps():8.6f}, beta={beta_scheduler.get_eps():8.6f}, kl={kl_upper_bound:10.5g}, "
...
def discounted_robust_ppo_step(all_states, actions, old_log_ps, rewards, returns, not_dones, advs, net, params, store, opt_step, relaxed_net, eps_scheduler, beta_scheduler):
    """
    Proximal Policy Optimization with robustness regularizer
    Runs K epochs of PPO as in https://arxiv.org/abs/1707.06347
    Inputs:
    ...
    for _ in range(params.PPO_EPOCHS):
        if params.HISTORY_LENGTH > 0:
            ...
            if params.ROBUST_PPO_METHOD == "sgld":
                ...
                kl_upper_bound_array = get_state_kl_bound_sgld(net, all_states, None, eps=current_eps, steps=params.ROBUST_PPO_PGD_STEPS, stdev=stdev, not_dones=not_dones)
            else:
                raise ValueError(f"Unsupported robust PPO method {params.ROBUST_PPO_METHOD}")
            # Compute time discounted kl upper bound
            timesteps = all_states[:, -1]  # last dimension of states are time in discounted mode.
            discounted_kl_upper_bound_array = kl_upper_bound_array * params.GAMMA**timesteps
            discounted_kl_upper_bound = discounted_kl_upper_bound_array.mean()

            entropy = -params.ENTROPY_COEFF * entropy_bonus
            loss = surrogate + entropy + params.ROBUST_PPO_REG * discounted_kl_upper_bound
            # optimizer (only ADAM)
            loss.backward()
            if params.CLIP_GRAD_NORM != -1:
                ch.nn.utils.clip_grad_norm(net.parameters(), params.CLIP_GRAD_NORM)
            ...
            for selected in splits:
                ...
                if params.ROBUST_PPO_DETACH_STDEV:
                    ...
                    stdev = stdev.detach()
                if params.ROBUST_PPO_METHOD == "convex-relax":
                    kl_upper_bound_array = get_state_kl_bound(relaxed_net, batch_states[..., :-1], batch_action_means, eps=current_eps, beta=beta_scheduler.get_eps(), stdev=stdev).mean()
                elif params.ROBUST_PPO_METHOD == "sgld":
                    kl_upper_bound_array = get_state_kl_bound_sgld(net, batch_states, batch_action_means, eps=current_eps, steps=params.ROBUST_PPO_PGD_STEPS, stdev=stdev).mean()
                else:
                    raise ValueError(f"Unsupported robust PPO method {params.ROBUST_PPO_METHOD}")
                # Compute time discounted kl upper bound
                timesteps = batch_states[:, -1]  # last dimension of states are time in discounted mode.
                discounted_kl_upper_bound_array = kl_upper_bound_array * params.GAMMA**timesteps
                discounted_kl_upper_bound = discounted_kl_upper_bound_array.mean()
                ...
                if params.POLICY_ADAM is None:
                    ...
                    params.POLICY_ADAM.step()
        # Logging.
        discounted_kl_upper_bound = discounted_kl_upper_bound.item()
        surrogate = surrogate.item()
        entropy_bonus = entropy_bonus.item()
        print(
            f"eps={eps_scheduler.get_eps():8.6f}, beta={beta_scheduler.get_eps():8.6f}, kl={discounted_kl_upper_bound:10.5g}, "
            f"surrogate={surrogate:8.5f}, entropy={entropy_bonus:8.5f}, loss={loss.item():8.5f}"
        )
    std = ch.exp(net.log_stdev)
    print(f"std_min={std.min().item():8.5f}, std_max={std.max().item():8.5f}, std_mean={std.mean().item():8.5f}")
...
def trpo_step(all_states, actions, old_log_ps, rewards, returns, not_dones, advs, net, params, store, opt_step):
    ...
    def fisher_product(x, damp_coef=1.0):
...
def step_with_mode(mode, adversary=False):
    STEPS = {
        ...
        "discounted_robust_ppo": discounted_robust_ppo_step,
        "adv_ppo": ppo_step,
        "adv_trpo": trpo_step,
