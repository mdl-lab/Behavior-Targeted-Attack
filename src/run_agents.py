...
def generate_attack_script(configs, args, ATTACK_THREADS):
...
def run_single_config(queue, index, args, args_params):
    ...
    while True:
        ...
        json_filename = path.splitext(path.basename(conf_path))[0]
        if args.out_dir_prefix:
            params["out_dir"] = path.join(args.out_dir_prefix, params["out_dir"], json_filename)
        else:
            params["out_dir"] = path.join(params["out_dir"], json_filename)
        try:
...
if __name__ == "__main__":
    ...
    parser.add_argument("config_path", type=str, nargs="+", help="json path to run all json files inside")
    ...
    parser.add_argument(
        "--no-load-adv-policy", action="store_true", required=False, help="Do not load adversary policy and value network from pretrained model."
    )
    ...
    if run_attack:
        attack_script = generate_attack_script(sorted_filelist, args, ATTACK_THREADS)

    time.sleep(5)

    for i in range(min(njobs, NUM_THREADS)):
        worker = Process(target=run_single_config, args=(q, i, args, args_params))
        worker.daemon = True
        worker.start()
