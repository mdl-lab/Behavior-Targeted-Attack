...
from run import add_common_parser_opts, main, override_json_params
...
def main(params):
    ...
    if params["sarsa_enable"]:
        assert params["attack_method"] == "none" or params["attack_method"] is None, "--train-sarsa is only available when --attack-method=none, but got {}".format(params["attack_method"])

    if params["save_normalization_factor"]:
        print("In test, use normalization factor")
        params["save_normalization_factor"] = False
    ...
    if params["sarsa_enable"]:
        ...
        for i in range(num_episodes):
            ...
            ep_length, ep_reward, actions, action_means, states, kl_certificates = p.run_test(
                compute_bounds=params["compute_kl_cert"], use_full_backward=params["use_full_backward"], original_stdev=original_stdev
            )
        ...
        for i in range(num_episodes):
            ...
            ep_length, ep_reward, ep_success, actions, action_means, states, kl_certificates = p.run_test(
                compute_bounds=params["compute_kl_cert"], use_full_backward=params["use_full_backward"], original_stdev=original_stdev
            )
            if i == 0:
                all_actions = actions.copy()
            ...
            if i > num_episodes // 5 and params["early_terminate"] and params["sqlite_path"] and params["attack_method"] != "none":
                ...
                if current_best_reward is not None and ((current_best_reward < mean_reward - 2 * std_reward) or (min_reward > current_best_reward)):
                    print("terminating early!")
                    early_terminate = True
                    break

        mean_reward, std_reward, min_reward, max_reward = np.mean(all_rewards), np.std(all_rewards), np.min(all_rewards), np.max(all_rewards)
        print("mean reward: {}, std: {}, min: {}, max: {}".format(mean_reward, std_reward, min_reward, max_reward))
        ...
        for name, value in [("actions", all_actions), ("states", all_states), ("rewards", all_rewards), ("length", all_lens), ("success", all_success)]:
        ...
        if params["compute_kl_cert"]:
            print(
                "KL certificates stats: mean: {}, std: {}, min: {}, max: {}".format(np.mean(all_kl_certificates), np.std(all_kl_certificates), np.min(all_kl_certificates), np.max(all_kl_certificates))
            )
        ...
        if params["json_path"] != "":
            json_results = {
                "rewards": {"mean": float(mean_reward), "std": float(std_reward), "min": float(min_reward), "max": float(max_reward)},
                "episode_lengths": {"mean": float(np.mean(all_lens)), "std": float(np.std(all_lens)), "min": float(np.min(all_lens)), "max": float(np.max(all_lens))},
            ...
            os.makedirs(params["json_path"], exist_ok=True)
            with open(os.path.join(params["json_path"], "results.json"), "w") as f:
                json.dump(json_results, f, indent=4)
        print("\n")
        print("all rewards:", all_rewards)
        print("rewards stats:\nmean: {}, std:{}, min:{}, max:{}".format(mean_reward, std_reward, min_reward, max_reward))
        print("\n")
        print("all success:", all_success)
        print("success rate: ", np.mean(all_success))
...
def get_parser():
    ...
    parser.add_argument("--json-path", type=str, help="save results to a json file.", default="")
